"""NOVA24 — Extended RV-gated overnight drift across SPY/QQQ/IWM.

NOVA23 proved the principle on SPY (OOS SR 0.99 with RV<0.15 gate).
Since the overnight drift anomaly is documented across all major US
equity indices (Lou-Polk-Skouras 2019 tests on SPY, QQQ, IWM separately)
we replicate the RV-gate strategy on each and ensemble.

Per-ticker RV<0.15 gate, 2 bps TC round-trip on overnight legs only.
Equal-weight across 3 tickers → each position is 1/3 of book.

Fixed threshold (0.15) from NOVA23 spec — NO re-tuning per ticker."""
...
def main():
    ...
    ew = sdf.mean(axis=1)
    s = stats(ew, "NOVA24 EW 3-ticker ovn")
    ...
    s = stats(erc, "NOVA24 ERC 3-ticker ovn")
    ...
    out = pd.DataFrame({"NOVA24": erc}).join(sdf, how="outer")
    out.to_csv("/home/user/bonds/data/results/nova24_returns.csv")
    print("\nSaved /home/user/bonds/data/results/nova24_returns.csv")
