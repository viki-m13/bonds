"""
POLARIS_DIVERSIFIED — Multi-Equity Variant of POLARIS
======================================================

This file is a variant of POLARIS where the VOLT_RP sleeve is expanded
from QLD-only to a 3-LETF equity basket (QLD + TQQQ + TECL) at smaller
per-asset target vols. Empirical motivation:

We tested expanding POLARIS's VOLT_RP equity sleeve from QLD-only to
include sector LETFs (TECL, SOXL, FAS, ERX), international (EDC, YINN),
and other large-cap LETFs. Results at vt=18% portfolio overlay:

    POL baseline (QLD20)                      SR 1.23  CAGR 24.5%  MDD -23.6%
    POL QLD20+TECL15                          SR 1.22  CAGR 24.5%  MDD -21.6%
    POL QLD15+TECL15+SOXL15                   SR 1.21  CAGR 24.2%  MDD -21.9%
    POL QLD15+TQQQ15+TECL15  (this variant)   SR 1.22  CAGR 24.5%  MDD -20.7%

Expanding the equity leg from QLD-only to {QLD, TQQQ, TECL} at 15%
target vol each gives a 3pp MDD improvement (-23.6% -> -20.7%) with
essentially identical Sharpe and CAGR. International (EDC, YINN) HURT
performance -- emerging markets underperformed Nasdaq through 2010-2024
-- so they're excluded.

All other mechanics identical to POLARIS baseline.

------------------------------------------------------------------------
ORIGINAL POLARIS DOCUMENTATION (other 3 sleeves unchanged):
------------------------------------------------------------------------
...
# --------------------------------------------------------------------------- #
S1_TVOL = {"QLD": 0.15, "TQQQ": 0.15, "TECL": 0.15, "TYD": 0.10, "UGL": 0.10}
S1_VOL_LB = 21
S1_GROSS_CAP = 1.5
...
def main(verbose: bool = True) -> dict:
    ...
    (RESULTS / "polaris_div_metrics.json").write_text(json.dumps(out, indent=2, default=float))
    pd.DataFrame({
    ...
    }).to_csv(RESULTS / "polaris_div_returns.csv", index=False)
    pd.DataFrame(thr_df).to_csv(RESULTS / "polaris_div_sleeves.csv")
    pd.DataFrame(raw_df).to_csv(RESULTS / "polaris_div_sleeves_raw.csv")
    if verbose:
        print(f"\nSaved: {RESULTS/'polaris_div_metrics.json'}")
        print(f"Saved: {RESULTS/'polaris_div_returns.csv'}")
        print(f"Saved: {RESULTS/'polaris_div_sleeves.csv'}")
    return out
