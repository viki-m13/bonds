"""APEX v24 — exhaustively search best minimal-sleeve combos.

Try ALL 3-sleeve, 4-sleeve, 5-sleeve combinations among the 10 strongest
individual sleeves. Pick the one with highest OOS Sharpe.
"""
...
# Candidate pool: 12 reasonable sleeves
POOL = {
...
def main():
    ...
    # Search: test all 3, 4, 5, 6-sleeve combos
    all_results = []
    config_idx = 0
    for k in [3, 4, 5]:
        combos = list(combinations(POOL.keys(), k))
        print(f"\nTesting {len(combos)} combos of size {k}...")
        for combo in combos:
            sw = {name: full_sw[name] for name in combo}
            # Use fixed params for speed
            net, _ = run_v17(cp, sw, crypto_w=0.55, target_vol=0.18)
            m = util.metrics(net)
    ...
    net.to_frame("apex_v24_ret").to_csv(OUT / "apex_v24_returns.csv")
    (OUT / "apex_v24_meta.json").write_text(json.dumps({
        "best_combo": overall_best["combo"],
        "best_oos_sr": overall_best["oos_sr"],
        "best_full_sr": overall_best["full_sr"],
        "top_5_combos": [[list(r["combo"]), r["oos_sr"], r["full_sr"]] for r in all_results[:5]],
