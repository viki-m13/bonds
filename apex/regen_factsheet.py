"""Regenerate the apex_factsheet_final.json from latest apex_final_returns.csv."""
...
def yearly_returns(r: pd.Series) -> list[dict]:
    ...
    for y, grp in r.groupby(r.index.year):
        ...
        out.append({"year": int(y), "ret": round(float(ret), 4), "vol": round(float(sd), 4),
                    "mdd": round(float(mdd), 4), "sharpe": round(float(sharpe), 3)})
    return out
...
def main():
    ...
    sleeves = {}
    try:
        R = pd.read_csv(OUT / "apex_final_sleeve_returns.csv", parse_dates=["Date"]).set_index("Date")
        for n in R.columns:
    ...
    data = {
        "name": "APEX",
        "tagline": "Five-Sleeve Leveraged-ETF Ensemble with Phoenix-Style Overlays",
        "description": "Equal-weight blend of 5 uncorrelated sleeves (trend, risk-safe, sector rotation, XGBoost ML, dip-buy) with vol-regime gate + DD throttle + vol target.",
