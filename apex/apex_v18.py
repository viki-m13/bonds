"""APEX v18 — add BREAKOUT52 + HMM + DIVERGENCE to v17."""
...
def main():
    ...
    print("Building v18 sleeves (16 LETF sleeves)...")
    sw = build_v18(cp)
    ...
    print(f"\nGreedy (10-max, corr_pen 0.8): {sel_greedy}")
    ...
    for cw in [0.35, 0.45, 0.50]:
        for tv in [0.18, 0.22]:
            ...
            configs.append({"sel": "all16", "cw": cw, "tv": tv,
    ...
    # Top-OOS subset (top 8 by IS-computed OOS SR)
    ...
    top8_by_is = is_sr_vec.sort_values(ascending=False).head(8).index.tolist()
    print(f"\nTop-8 by IS SR: {top8_by_is}")
    for cw in [0.35, 0.45, 0.50]:
        net, _ = run_v17(cp, sw, crypto_w=cw, target_vol=0.20, selected_sleeves=top8_by_is)
        m = util.metrics(net)
        om = util.metrics(util.regime_slice(net, OOS_START, "2027-12-31"))
        m22 = util.metrics(util.regime_slice(net, "2022-01-01", "2022-12-31"))
        configs.append({"sel": "top8IS", "cw": cw, "tv": 0.20,
                        "full_sr": m["sharpe"], "oos_sr": om.get("sharpe", 0),
                        "oos_cagr": om.get("cagr", 0), "y22": m22.get("sharpe", 0),
                        "mdd": m["mdd"], "net": net})
    ...
    net.to_frame("apex_v18_ret").to_csv(OUT / "apex_v18_returns.csv")
    (OUT / "apex_v18_meta.json").write_text(json.dumps({
        "best": {k: v for k, v in best.items() if k != "net"},
        "greedy_selected": sel_greedy,
        "top8_by_is": top8_by_is,
