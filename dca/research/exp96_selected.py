...
M=me.index
# strong discriminators from winner/loser study (exp94)
SEL=["roa","op_margin","net_margin","distHigh","vol6","mom12","rnd_int","share_chg",
     "mom6","log_mcap","rule40","roe","price_accel","trend","mom3","netcash_mcap"]
SEL=[s for s in SEL if s in FEAT]
Z={nm:FEAT[nm].where(liq).rank(axis=1,pct=True) for nm in SEL}
recs=[]
for dt in M[(M>=pd.Timestamp("2011-06-01"))]:
    ...
    X=np.column_stack([Z[nm].loc[dt].reindex(fv.index).values for nm in SEL])
    for i,tk in enumerate(fv.index): recs.append((dt,tk,*X[i],y.iloc[i]))
DF=pd.DataFrame.from_records(recs,columns=["date","tk"]+SEL+["y"]).fillna(0.5)
p(f"selected-feature table {DF.shape} ({len(SEL)} feats) t={time.time()-t0:.0f}s")
...
def wf(builder,name):
    ...
    for ytest in range(2015,2026):
        ...
        clf=builder(); clf.fit(tr[SEL].values,tr["y"].astype(int).values)
        t2=te[["date","tk"]].copy(); t2["p"]=clf.predict_proba(te[SEL].values)[:,1]; preds.append(t2)
    P=pd.concat(preds).pivot_table(index="date",columns="tk",values="p").reindex(M); p(f"  {name} t={time.time()-t0:.0f}s"); return P
P_gbt=wf(lambda:HistGradientBoostingClassifier(max_iter=300,max_depth=4,learning_rate=0.04,l2_regularization=1.0,min_samples_leaf=150,random_state=0),"GBT-sel")
P_et=wf(lambda:ExtraTreesClassifier(n_estimators=200,max_depth=12,min_samples_leaf=100,n_jobs=4,random_state=0),"ET-sel")
P_lr=wf(lambda:LogisticRegression(C=0.5,max_iter=200),"LR-sel")
ENS=( P_gbt.rank(axis=1,pct=True)+P_et.rank(axis=1,pct=True)+P_lr.rank(axis=1,pct=True) )/3
...
p(f"\n{'model':30} {'IC3m':>7} {'CAGR':>7} {'Sharpe':>7} {'maxDD':>7}")
c,s,d=stats(qret); p(f"{'QQQ':30} {'':>7} {c:>7.1%} {s:>7.2f} {d:>7.1%}")
for nm,P in [("old full-feature GBT",P_old),("GBT selected-16",P_gbt),("ET selected-16",P_et),("LR selected-16",P_lr),("ENS selected (gbt+et+lr)",ENS)]:
    r=champ(P); c,s,d=stats(r); p(f"{nm:30} {ic(P):>7.3f} {c:>7.1%} {s:>7.2f} {d:>7.1%}")
pd.to_pickle(ENS,"/tmp/wave/_mlprob_selens.pkl")
