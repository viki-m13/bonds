...
def show(nm,mask,reg,vt,pf,mp):
    c,s,dd,wr,n=run(mask,use_regime=reg,voltgt=vt,pos_frac=pf,maxpos=mp)
    print(f"  {nm:40}{c*100:>6.1f}%{s:>7.2f}{dd*100:>6.0f}%{wr*100:>5.0f}%{n:>7}",flush=True)
print(f"{'variant':42}{'CAGR':>6}{'Shrp':>7}{'maxDD':>7}{'winR':>6}{'nTrd':>7}")
show("base ever-member (pf20 mp10)", base_uni&trig3, False,None,0.20,10)
m10=base_uni&trig3&(rsi2<10); m15=base_uni&trig3&(rsi2<15)
for pf in [0.20,0.30,0.40,0.50]: show(f"RSI2<10 pf{int(pf*100)} mp10", m10,False,None,pf,10)
show("RSI2<10 pf30 mp12", m10,False,None,0.30,12)
show("RSI2<10 pf40 mp12", m10,False,None,0.40,12)
for pf in [0.30,0.40]: show(f"RSI2<15 pf{int(pf*100)} mp10", m15,False,None,pf,10)
show("RSI2<10 pf40 mp12 + vol-target25", m10,False,0.25,0.40,12)
