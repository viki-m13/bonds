...
# clean QQQ benchmark (retry)
qm=None
for _ in range(4):
    try:
        d=json.loads(urllib.request.urlopen('https://api.tiingo.com/tiingo/daily/QQQ/prices?startDate=2001-01-01&token='+os.environ['TIINGO']+'&format=json',timeout=90).read())
        qm=pd.Series({pd.Timestamp(x['date'][:10]):x['adjClose'] for x in d}).resample("ME").last().pct_change().reindex(S.index).dropna(); break
    except Exception: continue
if qm is not None: line("QQQ buy&hold (benchmark)",qm)
