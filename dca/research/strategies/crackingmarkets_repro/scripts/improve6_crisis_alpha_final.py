...
def vt(r,tgt=0.10,win=6,cap=2.5):
    rv=r.rolling(win,min_periods=3).std()*np.sqrt(12); lev=(tgt/rv).clip(upper=cap).shift(1).fillna(1.0); return r*lev
ew=S.mean(axis=1)                                  # equal weight
c64=0.6*S["MeanRev(improved)"]+0.4*S["Momentum(12-1)"]
print(f"\n{'weighting scheme':34}{'CAGR':>7}{'Sharpe':>8}{'maxDD':>7}{'devSh':>7}{'holdSh':>8}")
def line(nm,r):
    a,b,d=stat2(r); dv=stat2(r[dev])[1]; hl=stat2(r[hold])[1]
    print(f"  {nm:32}{a*100:>6.1f}%{b:>8.2f}{d*100:>6.0f}%{dv:>7.2f}{hl:>8.2f}")
line("equal-vol (risk-parity)",combo)
line("equal-weight",ew)
line("60/40 MeanRev/Momentum",c64)
line("equal-vol + vol-target 10%",vt(combo,0.10))
line("equal-vol + vol-target 12%",vt(combo,0.12))
import os,urllib.request,json,matplotlib; matplotlib.use("Agg"); import matplotlib.pyplot as plt
def dl(t):
    for _ in range(4):
        try:
            d=json.loads(urllib.request.urlopen(f'https://api.tiingo.com/tiingo/daily/{t}/prices?startDate=2001-01-01&token='+os.environ['TIINGO']+'&format=json',timeout=90).read())
            return pd.Series({pd.Timestamp(x['date'][:10]):x['adjClose'] for x in d}).resample("ME").last()
        except Exception: continue
    return None
# defensive crisis-alpha sleeve: trend-following on bonds+gold (long when 12-1 mom>0, else cash), equal-weight
px={t:dl(t) for t in ["TLT","IEF","GLD","QQQ"]}
D=pd.DataFrame({k:v for k,v in px.items() if v is not None})
mom=D/D.shift(12)-1; ret=D.pct_change()
defn=[]
for a in ["TLT","IEF","GLD"]:
    if a in D: defn.append((ret[a]*(mom[a].shift(1)>0)).fillna(0))
defsleeve=pd.concat(defn,axis=1).mean(axis=1).reindex(S.index).fillna(0)
S4=S.copy(); S4["Defensive(bond/gold trend)"]=defsleeve
print("4-sleeve correlation:\n",S4.corr().round(2).to_string())
iv=1/S4.std(); wv=iv/iv.sum(); combo4=(S4*wv).sum(axis=1)
qm=ret["QQQ"].reindex(S.index).dropna() if "QQQ" in ret else None
print(f"\n{'system':34}{'CAGR':>7}{'Sharpe':>8}{'maxDD':>7}{'devSh':>7}{'holdSh':>8}")
def line(nm,r):
    a,b,d=stat2(r); print(f"  {nm:32}{a*100:>6.1f}%{b:>8.2f}{d*100:>6.0f}%{stat2(r[dev])[1]:>7.2f}{stat2(r[hold])[1]:>8.2f}")
line("3-sleeve risk-parity (prev best)",combo)
line("4-sleeve + crisis-alpha (defensive)",combo4)
if qm is not None: line("QQQ buy&hold",qm)
# final chart: 4-sleeve combo vs 3-sleeve vs QQQ
fig,ax=plt.subplots(figsize=(11,5.5))
for r,c,l,nm in [(combo,"#888",1.6,"3-sleeve combo"),(combo4,"#111",2.6,"4-sleeve proprietary combo"),(qm,"#2563eb",1.6,"QQQ")]:
    if r is None: continue
    g=(1+r.reindex(S.index).fillna(0)).cumprod(); a,b,d=stat2(r)
    ax.plot(g.index,g,color=c,lw=l,ls=("--" if nm=="QQQ" else "-"),label=f"{nm} (Sh {b:.2f}, CAGR {a*100:.0f}%, DD {d*100:.0f}%)")
ax.set_yscale("log"); ax.grid(alpha=.3,which="both"); ax.legend(loc="upper left",fontsize=9)
ax.set_title("Final proprietary system: 4-sleeve risk-parity (mean-rev + momentum + buy-dip + crisis-alpha) vs QQQ")
fig.tight_layout(); fig.savefig("/home/user/combo_final.png",dpi=120); print("\nsaved /home/user/combo_final.png")
