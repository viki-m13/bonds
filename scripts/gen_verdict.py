...
br_ = E["breadth"]["rates"]
bl = E["blend"]
_e1 = bl["1999\u20132009 (the era QQQ lost)"]
_e2 = bl["2010\u20132026 (the era QQQ won)"]
c_blend1 = hbars_svg([
    ("QQQ", _e1["QQQ"]/1e3, "", "#111418"),
    ("50/50 blend", _e1["50/50"]/1e3, "never first, never last", "#6b7280"),
    ("SPY", _e1["SPY"]/1e3, "", "#374151"),
], xmax=160, fmt=lambda v: f"${v:,.0f}k")
c_blend2 = hbars_svg([
    ("QQQ", _e2["QQQ"]/1e3, "", "#111418"),
    ("50/50 blend", _e2["50/50"]/1e3, "never first, never last", "#6b7280"),
    ("SPY", _e2["SPY"]/1e3, "", "#374151"),
], xmax=1400, fmt=lambda v: f"${v:,.0f}k")
c2000 = bl["contributor2000"]
menu = E["menu"]
ef = E["eraflip"]
...
<h3>2.6 &nbsp;“Prove you're not just chasing past performance.” All right — here is the proof, and its limits</h3>
<p><b>First, the honest impossibility result.</b> Any argument concluding "QQQ is the best machine going forward" must rest on one of two things: its past returns — which is past-performance bias <i>by definition</i> — or a composition thesis ("a concentrated, cap-weighted index of the era's dominant growth companies"). So test that argument <i>form</i> at other dates, the way §5 tests strategies: in 1989 it selects <b>Japan</b> (three decades underwater); in March 2000 it selects <b>QQQ itself</b>, on the eve of −81%; in 2007 it overweights <b>financials</b>. An argument form that produces catastrophes at other evaluation dates is not a proof — it's a description of the recent winner. Conclusion, stated plainly: <b>no unbiased proof that "QQQ is best" exists. Anyone who offers one is selling §4's hot list at the index level.</b> This paper won't.</p>
<p><b>Second, the bias audit of this study itself</b> — how you can check we aren't doing covertly what we just called impossible:</p>
<ul>
<li>We showed the eras and slices where QQQ <i>lost</i> — the decade below SPY (§9's rotation chart), the sector fund that beat it (§2.5), gold out-compounding international equities (§2.4).</li>
<li>The verdict survives substituting the benchmark (§2.2, step 1) — it is about picking, not about QQQ.</li>
<li>The load-bearing mechanisms are identities and century-scale universals, not this sample (§2.2, step 2).</li>
<li><b>The tell that matters most: our null recommendation is not the past winner.</b> A performance-chaser would recommend the top of §2.5's chart — the tech sector fund. We don't; the no-forecast default in this paper is the <i>broadest</i> machine, which finished mid-table. Recommending something other than what performed best is what unbiased looks like.</li>
<li>The falsification conditions are published (end of §11), and this section adds one: show us the ex-ante argument for QQQ that survives the 1989/2000/2007 test, and we'll print it.</li>
</ul>
<p><b>Third, what a professional CAN derive without the bias</b> — three results and one measured discovery:</p>
<ul>
<li><b>The bet-free portfolio is the broadest cap-weight.</b> This is portfolio theory's one durable gift [7]: hold everything at market weights and you are, uniquely, betting on nothing except capitalism continuing. SPY / total-market is the practical instrument. <i>Every</i> narrower machine — QQQ included — is an active position against someone.</li>
<li><b>QQQ's honest forward case is an economic thesis, not a chart:</b> that value creation keeps concentrating in scalable, high-margin, network-effect businesses — the kind that list on its exchange. That is arguable economics; it is also known to every market participant and therefore already in prices. Holding QQQ is betting the concentration goes <i>further than priced</i>. Own that sentence or own SPY.</li>
<li><b>The 50/50 blend is not a cop-out; it's the decision-theory answer.</b> Under genuine uncertainty about the next era, minimizing your <i>maximum regret</i> across the two candidate machines lands near equal weights — and the naive 1/N split has embarrassed optimized portfolios in formal tests for decades [33]. Measured across both eras:
<div class="chart" style="margin-top:8px"><p style="font-weight:700;font-size:12.5px;margin:2px 0">1999–2009, the era QQQ lost (${_e1["contrib"]/1e3:,.0f}k contributed):</p>{c_blend1}<p style="font-weight:700;font-size:12.5px;margin:8px 0 2px">2010–2026, the era QQQ won (${_e2["contrib"]/1e3:,.0f}k contributed):</p>{c_blend2}</div>
<div class="leg"><span>Monthly-rebalanced 50/50; the blend is never first and never last in either era, and its full-period account drawdown (−43%) is the mildest of the three. It also owns the leader in <i>every</i> era — which matters because the measured first-order error is abandoning the plan, and people abandon plans that feel entirely wrong.</span></div></li>
<li><b>The measured discovery — DCA changes this choice:</b> a <i>contributor</i> interacts with volatility differently than a lump-sum holder. Even started at the worst month in QQQ's history (January 2000), the QQQ contributor's account fell to <b>{c2000["min_ratio"]}× of the SPY contributor's</b>, stayed behind for <b>{c2000["years_below"]} years</b> — and then finished at <b>{c2000["final_ratio"]}×</b>, because two decades of automatic purchases at crash prices converted the machine's volatility into shares. And in the 1999–2009 window above, the QQQ contributor edged the SPY contributor <i>even in QQQ's lost decade</i>. This does <b>not</b> prove QQQ superior — it is one path through one history. It proves something narrower and useful: <b>the concentrated machine's volatility costs a disciplined contributor less than it costs a lump-sum holder</b> — <i>if</i> the discipline holds through {c2000["years_below"]} years of watching a neighbor's simpler plan win.</li>
</ul>
...
<li><b>Winner-riding:</b> is it cap-weighted, letting winners grow to full weight — or does its rule trim winners (equal-weight, "rebalanced," most smart-beta)? <span class="note">(§3–4: the winners are everything)</span></li>
<li><b>Winner-keeping:</b> can it hold a winner forever — or does success eject the stock (small-cap indexes graduate winners out; dividend screens ejected the century's best compounders before they paid)? <span class="note">(§2.4)</span></li>
<li><b>Self-renewal:</b> does the rule automatically admit the <i>next</i> era's winners, whatever sector they're from? <span class="note">(§2.5: the index is a sector-rotator with hindsight because it needs no foresight)</span></li>
<li><b>Breadth at meaningful weight:</b> is the universe wide enough to contain the future giants? <span class="note">(evidence below — this one is subtler than it looks)</span></li>
<li><b>Rule predates results:</b> was the membership rule written before the performance it's sold on — or is it an outcome-defined basket / hype-timed launch? <span class="note">(§2.2, §11.5: thematic funds launch at peak excitement and shed ~6%/yr [23])</span></li>
<li><b>Cost &amp; structure hygiene:</b> expense ratio ≲0.2%, low internal turnover, tight index tracking, ETF tax structure, deep liquidity.</li>
<li><b>Holdability by you:</b> is its concentration-driven drawdown profile one <i>you</i> will hold through — the §11.2 stomach test, which is a property of the pairing of fund and owner, not of the fund alone.</li>
</ol></div>
<p><b>The breadth test, measured (and it surprises both sides):</b> of the 25 best-<i>performing</i> U.S. stocks of 2016–2026, only <b>{br_["top100"]}%</b> were top-100-sized at the start; {br_["top500"]}% were top-500; {br_["top1500"]}% top-1500. Narrow universes really do miss most of the return-lottery. <i>But</i> — cap-weighting holds those extra names at near-zero weight, so their fireworks barely move a broad fund; an index's return is made by companies that are <b>big while winning</b>, and the era's index-moving monsters (Nvidia at +18,180%, Broadcom, Tesla, Lilly) were all top-100 already. Net: breadth insurance is real but small, and concentration's cost is real but small — which is why test 4 nearly ties among the broad machines and the honest choice between them stays §2.6's labeled bet, not a qualification.</p>
<p><b>Now grade the menu</b> — every fund type from this paper, against the seven tests:</p>
<div class="chart"><table style="min-width:640px"><thead><tr><th>Fund type</th><th>1 Ride</th><th>2 Keep</th><th>3 Renew</th><th>4 Breadth</th><th>5 Rule-first</th><th>6 Cost</th><th>7 Hold</th><th>Verdict</th></tr></thead><tbody>
<tr><td><b>Total-market / SPY</b></td><td class="good">✓</td><td class="good">✓</td><td class="good">✓</td><td class="good">✓</td><td class="good">✓</td><td class="good">✓</td><td class="good">✓</td><td class="good">qualified — the neutral machine</td></tr>
<tr><td><b>QQQ</b></td><td class="good">✓</td><td class="good">✓</td><td class="good">✓</td><td>~</td><td class="good">✓</td><td class="good">✓</td><td>you?</td><td>qualified — with a declared concentration bet</td></tr>
<tr><td>Global cap-weight (VT-style)</td><td class="good">✓</td><td class="good">✓</td><td class="good">✓</td><td class="good">✓</td><td class="good">✓</td><td class="good">✓</td><td class="good">✓</td><td class="good">qualified — maximal humility</td></tr>
<tr><td>Equal-weight (RSP-style)</td><td class="bad">✗</td><td class="good">✓</td><td class="good">✓</td><td class="good">✓</td><td class="good">✓</td><td>~</td><td class="good">✓</td><td class="bad">trims winners quarterly — fails the core mechanism</td></tr>
<tr><td>Dividend / income funds</td><td class="bad">✗</td><td class="bad">✗</td><td class="bad">✗</td><td>~</td><td class="good">✓</td><td class="good">✓</td><td class="good">✓</td><td class="bad">excluded AAPL/AMZN/NVDA in their growth years</td></tr>
<tr><td>Small-cap index</td><td class="good">✓</td><td class="bad">✗</td><td class="good">✓</td><td>~</td><td class="good">✓</td><td class="good">✓</td><td>~</td><td class="bad">graduates every winner out</td></tr>
<tr><td>Single sector</td><td class="good">✓</td><td class="good">✓</td><td class="bad">✗</td><td class="bad">✗</td><td class="good">✓</td><td class="good">✓</td><td class="bad">✗</td><td class="bad">can't admit other sectors' winners; −41…−59% rides</td></tr>
<tr><td>Thematic / niche</td><td class="good">✓</td><td>~</td><td class="bad">✗</td><td class="bad">✗</td><td class="bad">✗</td><td class="bad">✗</td><td class="bad">✗</td><td class="bad">hype-timed launch, ~0.6% fees — fails 5 of 7</td></tr>
<tr><td>Leveraged funds</td><td class="good">✓</td><td class="good">✓</td><td class="good">✓</td><td>~</td><td class="good">✓</td><td class="bad">✗</td><td class="bad">✗</td><td class="bad">decay, fees, −84…−100% rides</td></tr>
</tbody></table></div>
<p><b>What the rubric settles — and what it can't.</b> It settles the fund <i>classes</i> decisively: only the broad cap-weighted machines pass all mechanical tests, and everything marketed loudly (thematic, dividend-income, equal-weight "smart" variants, leveraged) fails the very mechanisms §3–§5 measured. What it cannot settle is SPY-vs-QQQ, which come out as <i>the neutral machine</i> vs <i>the same machine with a declared bet</i> — precisely §2.6's conclusion, now derived from forward-checkable structure instead of any performance chart. That's the honest sense in which "which is best" <i>can</i> be qualified: <b>the class is provable; the point within the class is a preference plus a stomach.</b> And the rubric is portable — run the seven tests on the next ETF anyone pitches you; almost nothing sold with a story survives them.</p>
<h3>2.8 &nbsp;“If not one ETF — is there a best <i>combination</i> to DCA into forever?”</h3>
<p>Yes, there is a principled construction — and it follows from everything above rather than from any chart. A "forever" portfolio needs exactly two things:</p>
<div class="card"><p style="margin:4px 0"><b>An engine</b> — something that compounds and <i>self-renews</i> across eras. Only the broad cap-weighted machines pass that test (§2.7); nothing else both rides winners and admits the next era's winners automatically.</p>
<p style="margin:4px 0"><b>Optionally, a ballast</b> — a small holding whose only job is <i>keeping you holding the engine</i> through its worst valleys. A qualifying ballast must be genuinely uncorrelated in crashes, immune to obsolescence, and small enough not to dilute the engine.</p></div>
<p><b>Does gold qualify as the ballast? Measured:</b> its monthly correlation to QQQ over two decades is <b>{cmb["corr"]}</b> — essentially zero — and in QQQ's twelve <i>worst</i> months it averaged <b>+{cmb["gld_in_crash"]}%</b> while QQQ averaged {cmb["qqq_in_crash"]}%. That is real crisis insurance, from an asset with no earnings to disappoint and no counterparty to fail. (Its disqualification as an <i>engine</i> — §2.4 — is precisely why it must stay small.)</p>
<p><b>The measured combination menu</b> — identical $1,000/month, {cmb["window"]}, rebalanced monthly (${cmb["contrib"]/1e3:,.0f}k contributed):</p>
<table><thead><tr><th>Combination</th><th class="r">Final wealth</th><th class="r">Worst account fall</th></tr></thead><tbody>
{combo_rows}</tbody></table>
<p>Read the trade honestly: on this window, each 10% of gold surrendered roughly 8% of terminal wealth and bought ~3 points of drawdown relief — and the QQQ+gold mixes sat <i>above</i> the QQQ+SPY blend on both axes. Two required caveats before anyone over-reads that: <b>(1)</b> this window contains two strong gold eras (2005–11, 2024–26) — over 1980–2000 gold <i>lost roughly half its real value</i> while stocks compounded, which is what engine-lessness looks like when the insurance isn't paying; <b>(2)</b> the era logic of §2.6 applies to ballast too — gold's crisis behavior is its durable property, its this-window return is not.</p>
<p><b>The forever-construction, then, stated as a rule rather than a prediction:</b></p>
<ul>
<li><b>Engine, 90–100%:</b> the broad machine(s) of §2.6 — the 50/50 QQQ+SPY default, or 100% QQQ as the owned bet, or SPY/total-market as the no-bet purist choice.</li>
<li><b>Ballast, 0–10% gold:</b> include it <i>only</i> if §11.2's stomach test fails without it — its measured job is turning your worst months from {cmb["qqq_in_crash"]}% into roughly −9% and the deepest account valley from −33% into ~−30%, so that the automation survives you. If you'd hold through −33% anyway, the ballast is pure drag; skip it.</li>
<li><b>Never above ~20%:</b> beyond that you are no longer insuring an engine, you are replacing it with something that cannot compound — and over a long horizon that cost grows without bound.</li>
